...
export const getAllRelationArticleUser = (req, res, next) => {
  ...
  connection.query(query, (error, results) => {
    ...
    const totalViewsCount = results.reduce(
      (sum, row) => sum + row.viewsCount,
      0
    );
  ...
  try {
    ...
    connection.query(
      ...
      (error, articleResult) => {
        ...
        connection.query(
          "INSERT INTO user_articles SET ?",
          userArticle,
          (error, userArticleResult) => {
            if (error) {
              throw error;
            }
...
export const incrementArticleView = async (req, res, next) => {
  try {
    ...
    connection.query(updateViewsQuery, [id], (error, results) => {
      ...
      return res
        .status(200)
        .json({ message: "Article view count incremented successfully." });
    });
  } catch (error) {
    next(error);
  }
